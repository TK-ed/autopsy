...
`autopsy` ingests raw logs from multiple services, correlates them by timestamp, detects anomalies, and outputs a structured incident postmortem — all from your terminal.
...
- [ ] Custom log format config (`.autopsy.yaml`)
...
- `pip install autopsy-cli` working
- GitHub Actions CI passing
- 100% working on macOS + Linux

### Week 4: Launch
- **Hacker News**: `Show HN: autopsy – CLI that turns raw logs into postmortems`
...
3. Install: pip install autopsy-cli
...
> *"I built an open source CLI called autopsy — it ingests logs from multiple services, correlates them by timestamp, detects anomaly patterns like error spikes and cascading failures, and outputs a structured postmortem. It's on PyPI, has X GitHub stars, and I've had contributors from 3 countries send PRs. The hardest part was building the timestamp normalization layer — logs from different services use wildly different formats, and I had to make the auto-detection reliable enough that users never have to specify the format manually."*

That's a senior-level answer from someone with 2 years of experience.
