...
def run(
    files: Optional[List[str]] = typer.Option(
        None, "--file", "-f", help="Log file(s) to analyze"
    ),
    services: Optional[List[str]] = typer.Option(
        None, "--service", "-s",
        help="Override service name (matches positionally with --file)"
    ),
    docker: Optional[List[str]] = typer.Option(
        None, "--docker", "-d", help="Docker container name(s)"
    ),
    since: Optional[str] = typer.Option(
        None, "--since", help="Docker --since value e.g. 2h, 30m"
    ),
    ...
    from_dt: Optional[str] = typer.Option(
        None, "--from", help="Start of time window (YYYY-MM-DD HH:MM:SS)"
    ),
    to_dt: Optional[str] = typer.Option(
        None, "--to", help="End of time window (YYYY-MM-DD HH:MM:SS)"
    ),
    output: str = typer.Option(
        "terminal", "--output", "-o",
        help="Output format: terminal | markdown | json"
    ),
    ...
    if stdin:
        console.print(f"[dim]⌨️  Reading stdin...[/dim]")
...
def _print_terminal_summary(timeline, anomalies, services):
    ...
    if start and end:
        stats.add_row("Window",    f"[cyan]{start.strftime('%H:%M:%S')} → {end.strftime('%H:%M:%S')}[/cyan]")
    ...
    if error_events:
        ...
        for e in error_events:
            ...
            tbl2.add_row(ts_s, e.service, f"[{style}]{e.level}[/{style}]",
                         e.message[:100])
